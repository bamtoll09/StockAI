...
class Agent:
    # 에이전트 상태가 구성하는 값 개수
    STATE_DIM = # 주식 보유 비율, 포트폴리오 가치 비율
    
    # 매매 수수료 및 세금
    TRADING_CHARGE = 0 # 거래 수수료 미고려 (일반적으로 0.015%)
    TRADING_TAX =    0 # 서래세 미고려 (실제 0.3%)

    # 행동
    ACTION_BUY =  0 # 매수
    ACTION_SELL = 1 # 매도
    ACTION_HOLD = 2 # 관망
    ACTIONS = [ACTION_BUY, ACTION_SELL] # 인공신경망에서 확률을 구할 행동들
    NUM_ACTIONS = len (ACTIONS) # 인공신경망에서 고려할 출력값의 개수
    ...
    def __init__(
        ...
        self.base_portfolio_value = # 직전 학습 시점의 pv
    ...
    def get_states(self):
        ...
        self.ratio_portfolio_value = self.portfolio_value / self.base_portfolio_value # self.initial_balance # ?
        return (
            self.ratio_hold,
    ...
    def act(self, action, confidence):
        ...
        # 매도
            ...
            # 매도
            invest_amount = curr_price * (1 - (self.TRADING_TAX + self.TRADING_CHARGE))
                * trading_unit
            self.num_stocks -= trading_unit # 보유 주식 수를 갱신
            self.balance += invest_amount   # 보유 현금을 갱신
        ...
        # 즉시 보상 판단
        self.immediate_reward = 1 if profitloxx >= 0 else -1 # ?
