...
from polity_network import PolicyNetwork
...
class PolicyLearner:
    ...
    def __init__(self,  stock_code, chart_data, training_data = None,
        ...
        self.num_features = self.training_data.shape[1] + self.agent._STATE_DIM
    ...
    def fit(
        ...
        epoch_summary_dir = os.path.join(
            ...
            % (self.stock_code, settings.get_time_str())
        )
        ...
        for epoch in range(num_epoches):
            ...
            logger.info("[Epoch %s/%s]\tEpsilon:%.4f\t#Exp1.:%d/%d\t"
                        "#Buy:%d\t#Sell:%d\t#Hold:%d\t"
                        "#Stocks:%d\tPV:%s\t"
